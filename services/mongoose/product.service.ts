...
export class ProductService {
    ...
    async removePromotionFromProduct(product_id: string, promotion_id: string): Promise<IProduct | null> {
        if (!isValidObjectId(product_id) || !isValidObjectId(promotion_id)) {
            return Promise.resolve(null);
        }

        return this.productModel.findByIdAndUpdate(product_id, { $pull: { promotions: promotion_id } }).populate('promotions');
    }
